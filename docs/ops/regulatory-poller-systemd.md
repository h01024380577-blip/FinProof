...
- `{"skipped":N}` 이고 `poll_skipped`:
  - `excerpt_not_tracked` — 등록 제목이 "정식명 — 용도" 형태의 **큐레이션 발췌본**이라 의도적으로 제외(전문 대조 부적합). 정식 원문을 추적하려면 정식명 문서를 등록.
  - `law_id_unresolved` — `search_law`/`search_admin_rule`가 title로 못 찾음. 약칭은 `src/server/regulatory/regulatory-title-aliases.ts`의 alias 맵에 정식명을 추가(예: `여신전문금융업법 (원문)` → `여신전문금융업법`).
  - `ambiguous_match` — 검색 1위가 문서 title과 정식명이 다름(예: 은행업감독규정↔상호저축은행업감독규정). **안전을 위해 추적하지 않음**; audit의 `matchedTitle` 확인 후 title 정렬 또는 명시 매핑.
