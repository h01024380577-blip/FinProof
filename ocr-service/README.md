...
| 환경변수 | 기본값 | 설명 |
|---|---|---|
| `FINPROOF_OCR_PROVIDER` | (미설정) | **`http`** 일 때 ON (정식 값). `python_service`는 하위호환 별칭. **미설정이면 기존 동작 100% 동일** |
| `FINPROOF_OCR_ENDPOINT` | — | 서비스 베이스 URL (예: `http://localhost:8000`). `http`일 때 필수 |
