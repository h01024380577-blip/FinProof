...
> **현재 범위 — Phase 1**: 지식 문서 인제스천(`extractKnowledgeDocumentText`)만 이 서비스로
> 라우팅한다. 심사 파일 OCR(`review-analysis-pipeline.ts`의 `ExtractedDocument` 생성)은
> **Phase 2**에서 같은 서비스로 이관 예정 — 이번 변경에서는 건드리지 않는다(blast radius 최소화).
...
> **호환성 메모**: `python_service`는 분석 파이프라인의 `provider-config.ts`에는 미지의 값이라
> 그쪽에서는 안전하게 `deterministic`으로 폴백한다. 따라서 이 플래그를 켜도 **지식 인제스천
> 경로(Phase 1)만** 영향받고, 심사 파이프라인 OCR은 그대로다.
...
1. **Phase 2** — 심사 파일 OCR(`review-analysis-pipeline.ts`)도 이 서비스로 이관.
2. **한국어 도메인 OCR** — PaddleOCR / 파인튜닝 도입은 baseline 효과가 eval로 확인된 뒤.
3. **배포** — EC2/별도 호스팅은 PoC 효과 증명 후.
